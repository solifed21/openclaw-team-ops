...
const server = http.createServer(async (req, res) => {
  ...
  if (path === "/agents") {
    ...
    const html = layout("에이전트 관리",
      ...
      ${["orchestrator","planner","researcher","executor-general","executor-dev","executor-docs","executor-ops","executor-qa","reviewer","qa","approver","ops","analyst","scribe","commander"].map(r => `<option>${r}</option>`).join('')}
