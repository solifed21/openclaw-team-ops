...
const server = http.createServer(async (req, res) => {
  ...
  if (path === "/settings") {
    ...
    const html = layout("팀 설정", `<div class="card">${noteImport}${noteSync}<div class="muted">역할 정책/승인 정책/워크플로우 기본값 설정</div><form method="post" action="/import/openclaw-config" style="margin-top:10px"><button type="submit">openclaw.json 가져오기(import)</button></form><form method="post" action="/sync/openclaw-config" style="margin-top:10px"><button type="submit">openclaw.json 반영(sync)</button></form></div><div class="card"><h3>봇끼리 대화 활성화 체크리스트 (OpenClaw 문서 기준)</h3><ol><li><code>channels.discord.accounts.&lt;agentId&gt;.token</code>에 에이전트별 토큰 설정</li><li><code>channels.discord.guilds.&lt;guildId&gt;.channels.&lt;channelId&gt;.allow=true</code></li><li><code>bindings[]</code>에 각 에이전트별 <code>agentId/accountId/guildId/peer.id(channelId)</code> 매핑</li><li>필요시 채널 <code>requireMention</code> 완화 또는 activation 정책 조정</li><li>루프 방지 규칙(상호 무한응답 방지) 운영</li></ol><div class="muted">현재 대시보드 sync는 위 네이티브 키들만 반영함.</div></div>`);
    res.setHeader("Content-Type", "text/html; charset=utf-8");
    return res.end(html);
  }
